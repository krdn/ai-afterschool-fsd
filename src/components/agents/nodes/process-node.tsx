...
export const ProcessNode = memo(function ProcessNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`rounded-lg border-2 px-4 py-3 min-w-[160px] shadow-sm transition-all ${style} ${selected ? 'ring-2 ring-primary ring-offset-2' : ''}`}>
      <Handle type="target" position={Position.Top} className="!bg-slate-500 !w-2.5 !h-2.5 !border-2 !border-background" />
      ...
      <Handle type="source" position={Position.Bottom} className="!bg-slate-500 !w-2.5 !h-2.5 !border-2 !border-background" />
    </div>
  );
});
