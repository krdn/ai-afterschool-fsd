...
const components: Components = {
  h1: ({ children }) => (
    <h1 className="text-xl font-bold text-foreground mt-6 mb-3 pb-2 border-b border">
      {children}
    </h1>
  ),
  h2: ({ children }) => (
  ...
  code: ({ children }) => (
    <code className="px-1.5 py-0.5 bg-muted text-pink-600 rounded text-xs font-mono">
      {children}
    </code>
  ),
  ...
  table: ({ children }) => (
    <div className="my-3 overflow-x-auto rounded-md border border">
      <table className="w-full text-sm">{children}</table>
    </div>
  ),
  thead: ({ children }) => <thead className="bg-muted border-b border">{children}</thead>,
...
export function MarkdownRenderer({ content, className = "" }: Props) {
  ...
  return (
    <div className={`markdown-rendered prose prose-sm max-w-none text-foreground ${className}`}>
      <ReactMarkdown
